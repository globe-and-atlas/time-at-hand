...
## 0.1.2 common dial settings — 2026-09-25
Shared display settings are implemented in `face_render_full`: date order, hour leading zero, minute leading zero, light/dark theme and center pivot visibility. Meridian also supports configurable globe water and land palette colors through `face_globe_colors`. `face_render_custom` remains as the legacy wrapper with 0.1.1 defaults. Native status indicators are drawn after the bitmap layer: battery off/percent/thin-bar/low-only and an optional Bluetooth disconnect `x`. Theme swaps palette indices0 and1 after Meridian's globe overlay, so the globe shading is calculated once and then adapted to light or dark dial. Preview style arrays migrate from legacy lengths5 and8 to the new14-field array.
