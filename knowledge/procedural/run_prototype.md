...
## 2026-09-24 — 0.1.1 settings release
Run `python3 execution/generate_settings.py`, then `python3 execution/build_editions.py` to regenerate all five PBWs. The generated settings expose 1–8px widths, independent hour/minute label sizes and optional ticks. Verify with `python3 execution/verify_styles.py`, `python3 execution/verify_clear_points.py`, `python3 execution/verify_editions.py`, `python3 execution/verify_hemisphere.py` and `node execution/verify_location.js`. Run `python3 execution/publish_edition_branches.py --push` after committing main; it regenerates and force-pushes the four edition branches. Upload each resulting 0.1.1 PBW as a new release to the existing dashboard listing so UUIDs and direct links remain stable.

## 2026-09-25 — 0.1.2 shared settings release
Run `python3 execution/generate_settings.py`, then `python3 execution/build_editions.py` to regenerate all five PBWs. The generated settings expose shared light/dark theme, center pivot visibility, battery indicator, Bluetooth disconnect marker, hour/minute leading-zero format and date order. Verify with `python3 execution/verify_styles.py`, `python3 execution/verify_clear_points.py`, `python3 execution/verify_calendar.py`, `python3 execution/verify_editions.py`, `python3 execution/verify_hemisphere.py`, `node execution/verify_location.js` and the Pebble-tool Python bundle inspector. Upload each resulting 0.1.2 PBW as a new release to the existing dashboard listing so UUIDs and direct links remain stable.
