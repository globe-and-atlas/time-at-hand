...
const styleIds = ['timeFont','handColor','minuteColor','handWidth','minuteWidth','hourLabelSize','minuteLabelSize','showTicks','hourFormat','minuteFormat','dialTheme','showPivot','batteryMode','bluetoothMode'];
...
function restoreStyle() {
  ...
  try {
    ...
    const migrated = Array.isArray(saved) && saved.length === 5 ? saved.concat([0,0,0,0,1,0,1,0,1]) :
      Array.isArray(saved) && saved.length === 8 ? saved.concat([0,1,0,1,0,1]) : saved;
    if (Array.isArray(migrated) && migrated.length === styleIds.length && migrated.every((v,i) => (i===1 || i===2) ? Number.isInteger(v) && (v===-1 || (v>=1 && v<=73)) : $(styleIds[i]).type === 'checkbox' ? (v===0 || v===1) : Array.from($(styleIds[i]).options).some(o => String(v) === o.value))) s = migrated;
...
function updateStyleSummary() {
  ...
  const theme = $('dialTheme').value === '1' ? 'dark dial' : 'light dial';
  const status = $('batteryMode').value === '0' ? 'battery hidden' : $('batteryMode').selectedOptions[0].textContent.toLowerCase();
  $('styleSummary').textContent = `${font.group} / ${font.name}. ${width}; ${ticks}; ${theme}; ${status}.`;
}
...
function calendarSettings() {
  return { mask: dateFields.reduce((mask, id, i) => mask | ($(id).checked ? 1 << i : 0), 0), position: Number($('datePosition').value), format: Number($('dateFormat').value) };
}
...
function calendarLabel() {
  ...
  const ordered = s.format === 1 ? [parts[1],parts[2],parts[3],parts[0]] :
    s.format === 2 ? [parts[2],parts[1],parts[3],parts[0]] :
    s.format === 3 ? [String(d.getFullYear()).padStart(4,'0')+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0'),parts[0]] : parts;
  if (s.format === 3) return [s.mask & 14 ? ordered[0] : '', s.mask & 1 ? ordered[1] : ''].filter(Boolean).join(' ');
  const masks = s.format === 0 ? [1,2,4,8] : s.format === 1 ? [2,4,8,1] : [4,2,8,1];
  return ordered.filter((_, i) => s.mask & masks[i]).join(' ');
}
...
function restoreCalendar() {
  let s = { mask: 0, position: 0, format: 0 };
  try {
    const saved = JSON.parse(localStorage.getItem(`time-as-hand.calendar.${edition}`));
    if (saved && Number.isInteger(saved.mask) && saved.mask >= 0 && saved.mask <= 15 && [0,1].includes(saved.position)) s = {...s, ...saved};
  ...
  $('dateFormat').value = String(s.format || 0);
  updateCalendar();
}
function hourLabel(h) { return $('hourFormat').checked ? String(h).padStart(2,'0') : String(h); }
function minuteLabel(m) { return $('minuteFormat').checked ? String(m).padStart(2,'0') : String(m); }
function show(value) {
  ...
  const minutes = minuteLabel(m);
  const text = hourLabel(h) + ':' + minutes;
  ...
  $('face').alt = edition ? `Hour ${hourLabel(h % 12 || 12)} on the short hand; minute ${minutes} on the long hand` : special ? `Only upright ${h % 12 || 12} at its hour position` : `${hourLabel(h % 12 || 12)}:${minutes} along the rotating hour hand`;
  if (calendarLabel()) $('face').alt += `; ${calendarLabel()} at the ${$('datePosition').value === '0' ? 'top' : 'bottom'}`;
  $('description').textContent = edition ? `Hour ${hourLabel(h % 12 || 12)} on the short hand. Minute ${minutes} on the long hand.` : special ? 'A single upright numeral. The full time returns next minute.' : 'The full time follows the hour-hand position.';
...
function setEdition() {
  ...
  $('buildDownload').href = edition ? '/vector.pbw' : '/origin.pbw';
  $('buildDownload').download = edition ? 'vector.pbw' : 'origin.pbw';
  ...
  if(edition===3 || edition===4) {
    const name=edition===3 ? 'Cardinal' : 'Clarity',file=edition===3 ? 'cardinal' : 'clarity';
...
dateFields.concat(['datePosition','dateFormat']).forEach(id => $(id).addEventListener('change', () => {
...
try {
  ...
  if (saved && Number.isFinite(saved.lat) && Math.abs(saved.lat)<=90 && Number.isFinite(saved.lon) && Math.abs(saved.lon)<=180 && [0,1].includes(saved.valid)) globe={...globe,...saved};
...
$('locationPreset').addEventListener('change', () => {
  ...
  globe={...globe,lat:point[0],lon:point[1],valid:preset ? 1 : 0,preset};saveGlobe();
});
['latitude','longitude'].forEach(id => $(id).addEventListener('change', () => {
  ...
  globe={...globe,lat:Math.round(Number($('latitude').value)*10)/10,lon:Math.round(Number($('longitude').value)*10)/10,valid:1,preset:5};saveGlobe();
}));
...
$('locate').addEventListener('click', () => {
  ...
  navigator.geolocation.getCurrentPosition(p => {
    ...
    globe={...globe,lat:Math.round(p.coords.latitude*10)/10,lon:Math.round(p.coords.longitude*10)/10,valid:1,preset:5};saveGlobe('Approximate location saved. It stays on this device.');
