...
def load_renderer():
    ...
    lib.face_render_full.argtypes=[ctypes.c_int]*22+[ctypes.POINTER(ctypes.c_uint8)]
    lib.face_apply_theme.argtypes=[ctypes.c_int,ctypes.POINTER(ctypes.c_uint8)]
    lib.face_globe_colors.argtypes=[ctypes.c_int,ctypes.c_int]
    lib.face_globe_overlay.argtypes=[ctypes.c_int]*8+[ctypes.POINTER(ctypes.c_uint8)]
    return lib
def frame(lib,h:int,m:int,edition:int=0,mask:int=0,position:int=0,calendar:date|None=None,font:int=0,color:int=-1,minute_color:int=-1,width:int=0,minute_width:int=0,hour_label_size:int=0,minute_label_size:int=0,ticks:int=0,date_format:int=0,hour_format:int=0,minute_format:int=1,theme:int=0,pivot:int=1,utc:float|None=None,lat:float=0,lon:float=0,marker:int=0,water_color:int=0,land_color:int=7)->bytes:
    ...
    if not 0<=font<12 or color not in [-1,*range(1,9),*range(10,74)] or minute_color not in [-1,*range(1,9),*range(10,74)] or water_color not in range(74) or land_color not in range(74) or not 0<=width<=8 or not 0<=minute_width<=8 or not 0<=hour_label_size<=3 or not 0<=minute_label_size<=3 or ticks not in (0,1) or not 0<=date_format<=3 or hour_format not in (0,1) or minute_format not in (0,1) or theme not in (0,1) or pivot not in (0,1): raise ValueError('Invalid style')
    d=calendar or date.today()
    pixels=(ctypes.c_uint8*(W*H))()
    render_theme=0 if edition==2 else theme
    lib.face_render_full(h,m,edition,d.year,d.month,d.day,(d.weekday()+1)%7,mask,position,font,color,minute_color,width,minute_width,hour_label_size,minute_label_size,ticks,date_format,hour_format,minute_format,render_theme,pivot,pixels)
    if edition==2:
        instant=datetime.fromtimestamp(utc,timezone.utc) if utc is not None else datetime.now(timezone.utc)
        lib.face_globe_colors(water_color,land_color)
        ...
        lib.face_apply_theme(theme,pixels)
    return bytes(pixels)
        ...
        def do_GET(self):
            ...
            try:
                if url.path=='/face.png':
                    ...
                    style={key:int(q.get(key,[str(default)])[0]) for key,default in [('font',0),('color',-1),('minute_color',-1),('width',0),('minute_width',0),('hour_label_size',0),('minute_label_size',0),('ticks',0),('date_format',0),('hour_format',0),('minute_format',1),('theme',0),('pivot',1),('water_color',0),('land_color',7)]}
                ...
                elif url.path in ('/origin.pbw','/vector.pbw','/meridian.pbw','/cardinal.pbw','/clarity.pbw'):
                    name={'/origin.pbw':'origin.pbw','/vector.pbw':'vector.pbw','/meridian.pbw':'meridian.pbw','/cardinal.pbw':'cardinal.pbw','/clarity.pbw':'clarity.pbw'}[url.path]
